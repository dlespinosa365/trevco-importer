...
class AllFlowsPage extends Page implements HasTable
{
    use InteractsWithTable;

    protected static ?string $title = 'Flows';

    protected static ?string $slug = 'flows';
                    ...
                    ->form([
                        ToggleButtons::make('value')
                            ...
                            ->options(function (): array {
                                $options = ['all' => 'All'];
                                foreach (app(FlowDefinitionRegistry::class)->allIntegrations() as $row) {
                ...
                Filter::make('group_slug')
                    ...
                    ->form([
                        ToggleButtons::make('value')
                            ...
                            ->options(function (): array {
                                $options = ['all' => 'All'];
                                foreach ($this->availableGroupSlugsForFilters() as $groupSlug) {
            ...
            ->actions([
                ...
                Action::make('viewErrorLogs')
                    ...
                    ->url(fn (array $record): string => FlowErrorLogsPage::getUrl([
                        'integrationSlug' => $record['integration_slug'],
                ...
                Action::make('runNow')
                    ...
                    ->action(function (array $record): void {
                        ...
                        ExecuteIntegrationFlowJob::dispatchQueued(
                            ...
                            triggerPayload: [
                                'source' => 'filament_all_flows_page',
                            ],
                        );
    ...
    {
        return $this->flowListingRowBuilder()->buildRows($this->currentIntegrationFilter(), $this->currentGroupFilter());
    }
    ...
    {
        $value = data_get($this->tableFilters ?? [], 'integration_slug.value', 'all');
     ...
     * @return list<string>
     */
    private function availableGroupSlugsForFilters(): array
    {
        $integration = $this->currentIntegrationFilter();
        if ($integration === 'all') {
            return [];
        }

        return $this->flowListingRowBuilder()->groupSlugsForIntegration($integration);
    }
}
